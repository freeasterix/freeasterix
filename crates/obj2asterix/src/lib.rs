...
use spec_parser::spec_xml::{Category, Variable, Explicit, Compound, Format, Fixed};
...
fn calculate_fspec<'a>(
    ...
    for (key, value) in json.iter() {
        if key == "CAT" { continue; }
        let uap_item = uap.items.iter().find(|uap| &uap.name == key)
...
fn write_variable(
    ...
    for (idx, item) in variable.formats.iter().enumerate() {
        ...
        for bits in &fixed.bits {
            if field.contains_key(&bits.short_name) {
...
fn write_compound(
    ...
    for (byte_no, item) in head.formats.iter().enumerate() {
        ...
        for bits in fixed.bits.iter() {
            ...
            if let Some(value) = field.get(key) {
                ...
                let index = bits.presence
                    .ok_or_else(|| "Subitem specifier doesn't have BitsPresence".to_string())?;
                present_items.push(PresentItem { frn, key, value, index });
            }
        }
    }
    ...
    for &PresentItem { value, index, .. } in &present_items {
        let field = value.as_object()
            .ok_or_else(|| "Values on Compound keys must be maps".to_string())?;
        let format = compound.formats.get(index)
...
fn write_field(
    ...
    match &format {
        ...
        Format::Repetitive(_) => Err("Repetitive not implemented".to_string()),
...
pub fn write_asterix(
    ...
    for PresentItem { value, index, .. } in present_items {
        ...
        write_field(writer, &data_item.format.format, field)?;
    }

    let written_bytes = writer.len() - start;
    if written_bytes >= (1 << 16) {
        return Err("too many bytes written".to_string());
    }
...
mod tests {
    ...
    fn make_data() -> Value {
        serde_json::from_str(r#"
        {
          ...
          "105": {
              "Lat": -8041.411768198013,
              "Lon": 10119.52940940857
          }
        }
       "#).unwrap()
    }
