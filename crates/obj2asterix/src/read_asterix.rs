...
/// Reads one data record from byte array and converts it to JSON using provided specification.
/// 
/// To parse multiple data records at once use [`read_asterix_multi`].
/// 
/// Specification category can be generated by [`spec_parser`] crate.
/// 
/// Usage example:
/// ```
/// # use std::fs;
...
/// Reads multiple data records from byte array and converts it to a vector of JSON maps using provided specification.
/// 
/// Specification category can be generated by [`spec_parser`] crate.
/// 
/// Usage example:
...
/// # use std::fs;
/// use spec_parser::spec_xml::Category;
/// use obj2asterix::read_asterix_multi;
///
/// let xml = fs::read_to_string("../../specs-xml/asterix_cat062_1_18.xml")?;
/// let category = Category::parse(&xml)?;
/// let mut reader = &[0x3E, 0x00, 0x09, 0x80, 0xB0, 0xB1, 0x80, 0xB0, 0xB1][..];
/// let value = read_asterix_multi(&mut reader, &category)?;
/// dbg!(value);
/// # Ok::<(), Box<dyn std::error::Error>>(())
/// ```
