...
fn check_json() {
    ...
    let list = std::fs::read_to_string(list_path).expect("Cannot read list of JSON spec files");

    for fname in list.lines() {
        eprintln!("Reading spec `{fname}`");
        let spec_path = json_root.join(fname);
        let spec_json = std::fs::read_to_string(spec_path).expect("Cannot read spec JSON");
        if fname.contains("refs") {
            let _item: ExpansionCategory =
                serde_json::from_str(&spec_json).expect("Cannot parse ExpansionCategory");
        } else {
            let _item: BasicCategory =
                serde_json::from_str(&spec_json).expect("Cannot parse BasicCategory");
        }
    }
}
